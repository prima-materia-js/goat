...
describe('Edges', () => {
  ...
  test('can query inverse edges', async () => {
    const todoList = await TodoList.create()
      .set('listName', 'Example list')
      .save();
    ...
    const queriedList = await TodoList.queryInverseOfEdge(
      'items',
      todoItem
    ).getFirst();
    expect(queriedList).not.toBeNull();
    expect(queriedList?.getID()).toBe(todoList.getID());
  });

  test('inverse edge query handles non-existent edges', async () => {
    const todoItem = TodoItem.create().set('title', 'Do something');
    ...
    expect(queriedList).toBeNull();
  });

  test('inverse edge query throws on invalid edge name', async () => {
    const todoItem = TodoItem.create().set('title', 'Do something');

    expect(() => {
      TodoList.queryInverseOfEdge('not_a_valid_edge', todoItem);
    }).toThrow();
  });
});
